...
export const JobDetails: React.FC<JobDetailsProps> = ({ job, explanation, onExplain, isExplaining, onShowTopology, clusterName }) => {
        ...
        <div className="space-y-8 text-sm">
            ...
            {showTopology && clusterName && (
                <div className="border border-purple-500/30 rounded-lg overflow-hidden bg-black/20">
                    <ResourceTopology clusterName={clusterName} resource={job} />
                </div>
            )}
            ...
            <div>
                ...
                <div className="bg-white/5 rounded-md overflow-hidden border border-white/10">
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        ...
                        <span className="text-white">{status.active || 0}</span>
                     </div>
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Succeeded</span>
                        <span className="text-white">{status.succeeded || 0}</span>
                     </div>
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Failed</span>
                        <span className="text-white">{status.failed || 0}</span>
                     </div>
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Start Time</span>
                        <span className="text-white">{status.startTime ? new Date(status.startTime).toLocaleString() : '-'}</span>
                     </div>
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Completion Time</span>
                        <span className="text-white">{status.completionTime ? new Date(status.completionTime).toLocaleString() : '-'}</span>
                     </div>
                     
                     <div className="p-3 border-b border-white/10 bg-white/5">
                        <span className="text-gray-400 block mb-1">Spec</span>
                     </div>

                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Completions</span>
                        <span className="text-white">{spec.completions}</span>
                     </div>
                     <div className="p-3 border-b border-white/10 flex justify-between">
                        <span className="text-gray-400">Parallelism</span>
                        <span className="text-white">{spec.parallelism}</span>
                     </div>
                     <div className="p-3">
                        <span className="text-gray-400 block mb-1">Backoff Limit</span>
                        <span className="text-white">{spec.backoffLimit}</span>
                     </div>
                </div>
            </div>
